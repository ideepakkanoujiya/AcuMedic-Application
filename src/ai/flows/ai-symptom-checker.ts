...
const AISymptomCheckerOutputSchema = z.object({
  response: z.string().describe('The detailed, conversational response to the user query.'),
});
...
If the user describes symptoms, perform the following tasks in your response:
...
If the user asks a general question, provide a clear, helpful, and conversational answer.

Always maintain a supportive and professional tone. Start your first response with a greeting.
...
Your response must be in the structured format defined by the output schema, under the 'response' field.
`,
});
